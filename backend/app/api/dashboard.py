...
def get_dashboard_summary(
    ...
    current_month_transactions = (
        db.query(Transaction.amount, Category.type, Transaction.transaction_date)
        .join(Category)
        .filter(
    ...
    current_income = sum(
        t.amount for t in current_month_transactions if t.type == "INCOME"
    )
    current_expenses = sum(
        abs(t.amount) for t in current_month_transactions if t.type == "EXPENSE"
    )

    # 3. Last Month Income vs Expenses
    last_month_transactions = (
        db.query(Transaction.amount, Category.type)
        .join(Category)
        .filter(
            Transaction.user_id == current_user.id,
    ...
    last_month_expenses = sum(
        abs(t.amount) for t in last_month_transactions if t.type == "EXPENSE"
    )
    ...
    daily_expense_rate = (
        float(current_expenses) / current_day if current_day > 0 else 0.0
    )
    month_balance = float(current_income) - float(current_expenses)

    income_trend = (
        ((float(current_income) - float(last_month_income)) / float(last_month_income))
        * 100
