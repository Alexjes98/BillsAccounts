...
class User(Base):
    ...
    persons = relationship(
        "Person", foreign_keys="[Person.user_id]", back_populates="user"
    )
...
class Person(Base):
    ...
    user = relationship(
        "User", foreign_keys="[Person.user_id]", back_populates="persons"
    )
    debts_as_creditor = relationship(
        "Debt", foreign_keys="[Debt.creditor_id]", back_populates="creditor"
    )
    debts_as_debtor = relationship(
...
class Account(Base):
    ...
    )  # cash, bank, receivable, payable, credit_card
...
class Debt(Base):
    ...
    creditor = relationship(
        "Person", foreign_keys=[creditor_id], back_populates="debts_as_creditor"
    )
    debtor = relationship(
        "Person", foreign_keys=[debtor_id], back_populates="debts_as_debtor"
    )
    transactions = relationship("Transaction", back_populates="debt")

    __table_args__ = (Index("idx_debts_status", "user_id", "is_settled"),)
...
class MonthlySummary(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "year", "month", name="uq_monthly_summaries_user_year_month"
        ),
    )
